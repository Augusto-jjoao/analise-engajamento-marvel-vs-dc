movie_data = {
    "Marvel": {
        # Fase 4
        "Black Widow": "RxAtuMu_ph4",
        "Shang-Chi": "giWIr7U1deA",
        "Eternals": "0WVDKZJkGlY",
        "Spider-Man: No Way Home": "rt-2cxAiPJk", 
        "Doctor Strange 2": "aWzlQ2N6qqg",
        "Thor: Love and Thunder": "tgB1wUcmbbw",
        "Black Panther: Wakanda Forever": "RlOB3UALvrQ",
        
        # Fase 5
        "Ant-Man and the Wasp: Quantumania": "ZlNFpri-Y40",
        "Guardians of the Galaxy Vol. 3": "u3V5KDHRQvk",
        "The Marvels": "iuk77TjvfmE",
        "Deadpool & Wolverine": "73_1biulkYk",
        
        # Sony / Outros
        "Venom: Let There Be Carnage": "-ezfi6FQ8Ds",
        "Morbius": "oZ6iiRrz1SY",
        "Spider-Man: Across the Spider-Verse": "cqGjhVJWtEg",
        "Madame Web": "s_76M4c4LTo",
        "Kraven the Hunter": "rze8QYwWGMs", # Trailer Red Band
        "Venom: The Last Dance": "__2bjWbetsA"
    },
    
    "DC": {
        # DCEU (Final)
        "The Suicide Squad": "jz1FrtMIh5c",
        "Black Adam": "X0tOpBuYasI",
        "Shazam! Fury of the Gods": "Zi88i4CpHe4",
        "The Flash": "9vwaD9cHLNw",
        "Blue Beetle": "vS3_72Gb-bI",
        "Aquaman and the Lost Kingdom": "FV3bqvOHRQo",
        
        # Elseworlds (Outros universos)
        "Justice League": "3cxixDgHUYw",
        "The Batman": "mqqft2x_Aa4",
        "Joker: Folie à Deux": "xy8aJw1vYHo",
        
        # DCU (Novo Universo James Gunn)
        "Superman (2025)": "Ox8ZLF6cGM0" # Teaser/First Look (se houver, caso contrário remover)
    }
}

# Lista auxiliar para facilitar iterações se precisar
all_franchises = ["Marvel", "DC"]